...
Future<int> generateRoomNumber(SupabaseClient supabase) async {
  ...
  while (true) {
    ...
    final records2 = await supabase
        .from('Chat_room')
        .select('Room_number')
        .eq('Room_number', ID_select);
...
// 募集開始
Future<String> add_join(String Place, String Max_people, String Time,
    String Comment, SupabaseClient supabase) async {
  final response = await supabase
      .from('Chat_room')
      .select()
      .eq('leader_ID', login_num.now_login_ID);

  String Message = "";

  if (response.isNotEmpty) {
    Message = 'You have already added a place.'; //あなたは既に募集しています。
  } else {
    // チャットルームを作成
    ...
    Message = "Add successfully!"; //募集を開始します！
  }
