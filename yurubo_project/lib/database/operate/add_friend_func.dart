...
Future<String> add_friend(String new_friend_ID, SupabaseClient supabase) async {
  final record =
      await supabase.from('Profile').select('ID').eq('ID', new_friend_ID);

  String Message = "";
  if (record.isEmpty) {
    Message = 'No such preson.'; //errormessage1
  } else {
    final record_friend = await supabase
        .from('Friend')
        .select('friend_ID')
        .eq('friend_ID', new_friend_ID)
        .eq('ID', login_num.now_login_ID);

    // 入力IDが登録済みではないかをチェック
    if (record_friend.isNotEmpty) {
      Message = 'You have already added the friend.'; //その友達は既に登録済みです。
    } else {
      // 入力IDが自分ではないかをチェック
      if (login_num.now_login_ID == int.parse(new_friend_ID)) {
        Message = 'You can not add yourself'; //それはあなたのIDです。
      } else {
        ...
        final user = {
          ...
          'friend_ID': record[0]["ID"]
        };
        final user_reverse = {
          'ID': record[0]["ID"],
          'friend_ID': login_num.now_login_ID
        };
        await supabase.from('Friend').insert([user]);
        await supabase.from('Friend').insert([user_reverse]);

        Message = record[0]["ID"] + "Add successfully!"; //友達追加に成功しました。
      }
    }
